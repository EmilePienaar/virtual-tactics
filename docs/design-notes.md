...
## Installing without eating anyone's data

A symbiote folder is not only the symbiote. TaleSpire keeps the player's saved
characters in `.localStorage` **inside** it, and it is also where a connected
5etools `data/` directory and any imported `homebrew/` live. So the obvious
install — delete the folder, copy the new one — throws away every character at
the table and the data set they were built from. The handover note used to say
"copy file by file, never replace the folders" and trust everyone to remember.

`tools/install-symbiotes.js` does it properly:
...
node tools/install-symbiotes.js --prune    also clear generated files we dropped
node tools/install-symbiotes.js --to "..." when TaleSpire is somewhere unusual
```

`install.cmd` at the repo root wraps it for a double-click on Windows.

**Out of date means a byte difference.** Not the manifest's version, which is
written by hand and would sit at 1.0 through a hundred real changes, and not a
modification time, which a copy or a checkout resets for reasons that have
nothing to do with content. Files are hashed, size first as a cheap reject.

**It only ever adds and updates.** `data/`, `.localStorage` and `.debug` are
never touched, and neither is any file the build did not produce. Afterwards it
re-reads everything it wrote and compares again, so a file TaleSpire had open
and locked is reported rather than assumed.

### The pruning trap

`--prune` clears files a build no longer produces, which otherwise linger and
can still be loaded. It began by treating both `srd/` and `homebrew/` as
build-managed, since the build writes into both.

That was wrong, and a sandbox caught it: `homebrew/` is a **shared** space — the
build drops a converted supplement there and the player imports their own
beside it. The first `--prune` run in the test deleted the player's file. Only
`srd/` is prunable now, every file of which is generated by `extract-srd.js`,
and the installer reports any homebrew of the player's own that it is keeping.

---

## Folding the long cards away

A prepared caster's spell list is most of the sheet, and a player who has
finished casting for the night wants it out of the way. Actions and Spells fold,
and so does each spell level on its own.

The state has to persist or it is worse than useless — this sheet re-renders
constantly, and a fold that springs open on the next render is a fold that does
not work. It lives on `S.fold` and is saved with everything else, stored as *is
it shut* so that a section nobody has touched reads as open and a new character
starts with everything visible.

`<details>` rather than a class toggle, so the browser handles the keyboard, the
marker and the accessibility, and because the sheet already styles details that
way for Skills and Features.

One catch worth knowing: a spell level's summary holds the **live slot row**,
with its +/- buttons. A click on those has to be stopped before it reaches the
summary, or spending a slot folds the level shut underneath the finger that
spent it.

### "1·3" was two facts in four characters

The cast-at buttons read `1·3` — first level, three slots left — which looked
like a range and told you something the level's own row says a few pixels above.
They read `1st`, `2nd`, `3rd` now, and the count stays in the tooltip.
